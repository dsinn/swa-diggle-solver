...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result: Result<(NeedleResult, NeedleResult, usize), Box<dyn std::error::Error>> = (|| unsafe {
        ...
        eprintln!("DEBUG hpc.0={:#x} pid={} tid={}", hpc.0, pi.dwProcessId, pi.dwThreadId);
        ...
        while started.elapsed() < DEADLINE && (r1.found_at.is_none() || r2.found_at.is_none()) {
            if let Ok(chunk) = rx.recv_timeout(Duration::from_millis(250)) {
                ...
                if r1.found_at.is_none() && acc.contains(NEEDLE_1) {
                    ...
                    r1 =
                        NeedleResult { found_at: Some(started.elapsed()), alive_when_found: alive };
                }
